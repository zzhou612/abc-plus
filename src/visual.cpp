...
namespace abc {
    ...
    static int PLUS_Abc_NtkCountLogicNodes(Vec_Ptr_t *vNodes) {
        Abc_Obj_t *pObj;
        int i, Counter = 0;
    ...
    static void
    PLUS_Io_WriteDotNtk(Abc_Ntk_t *pNtk, Vec_Ptr_t *vNodes, Vec_Ptr_t *vNodesShow, char *pFileName, int fGateNames, int fUseReverse) {
        FILE *pFile;
        ...
        // start the stream
        if ((pFile = fopen(pFileName, "w")) == nullptr) {
        ...
        Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pNode, i)pNode->fMarkC = 1;
        if (vNodesShow)
            Vec_PtrForEachEntry(Abc_Obj_t *, vNodesShow, pNode, i)pNode->fMarkB = 1;
        ...
        fprintf(pFile, "\\n");
        fprintf(pFile, R"(Benchmark \"%s\". )", pNtk->pName);
        ...
        if (Abc_NtkObjNum(pNtk) == Vec_PtrSize(vNodes))
            ...
            fprintf(pFile, "The set contains %d logic nodes and spans %d levels.", PLUS_Abc_NtkCountLogicNodes(vNodes),
                    LevelMax - LevelMin + 1);
        fprintf(pFile, "\\n");
        fprintf(pFile, "\"\n");
        fprintf(pFile, "         ];\n");
        ...
        for (Level = LevelMax - fHasCos; Level >= LevelMin && Level > 0; Level--) {
            ...
            Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pNode, i) {
                ...
                if (Abc_NtkIsStrash(pNtk))
                    ...
                    pSopString = PLUS_Abc_NtkPrintSop(Mio_GateReadSop((Mio_Gate_t *) pNode->pData));
                else
                    pSopString = PLUS_Abc_NtkPrintSop((char *) pNode->pData);
                fprintf(pFile, R"(  Node%d [label = "%s\n%s")", pNode->Id, Abc_ObjName(pNode), pSopString);
...
//            fprintf( pFile, ", label = \"%c\"", 'a' + k );
        ...
        Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pNode, i)pNode->fMarkC = 0;
        if (vNodesShow)
            Vec_PtrForEachEntry(Abc_Obj_t *, vNodesShow, pNode, i)pNode->fMarkB = 0;
...
namespace abc_plus {
    void NtkWriteDot(NtkPtr ntk, const std::string &o_file) {
        ...
        vNodes = Abc_NtkCollectObjects(ntk);
        PLUS_Io_WriteDotNtk(ntk, vNodes, nullptr, (char *) o_file.c_str(), 0, 0);
        Vec_PtrFree(vNodes);
    }
}
